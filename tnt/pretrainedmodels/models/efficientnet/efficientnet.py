...
model_names = list(default_cfgs.keys())
...
def _create_model(model_kwargs, default_cfg, pretrained=False):
    ...
    # warning: pretrained can be string, True, rather than [], None, 0.
    if pretrained:
        load_pretrained(
            model,
            default_cfg,
    ...
    model.input_size = default_cfg["input_size"]
    model.mean = default_cfg["mean"]
    model.std = default_cfg["std"]
    return model
